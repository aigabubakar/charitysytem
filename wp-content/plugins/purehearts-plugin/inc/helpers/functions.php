...
function purehearts_the_pagination2($args = array(), $echo = 1)
{
...
add_filter('purehearts_redux_custom_fonts_load', 'purehearts_redux_custom_fonts_load');
function purehearts_redux_custom_fonts_load( $custom_font ) {
	$custom_style = '';
	...
	if ( $filename = purehearts_set( $pathinfo, 'filename' ) ) {
		...
		foreach ( $extensions as $extension ) {
			$file_path = esc_url(home_url('/')) . '/wp-content/themes/purehearts/assets/css/custom-fonts/' . $filename . '.' . $extension;
...
/**
 * [purehearts_social_share_output description]
 *
 ...
 */
function purehearts_social_share_output( $icon, $color = false ) {
...
function purehearts_get_social_icons()
{
	...
	if( purehearts_set( $options, 'social_media' ) && is_array( purehearts_set( $options, 'social_media' ) ) )
	{
		$total = count( purehearts_set( $options, 'social_media' ) ) - 2;
		foreach( purehearts_set( $options, 'social_media' ) as $social_icon ){
			if( isset( $social_icon['tocopy' ] ) ) continue;
			$title = purehearts_set( $social_icon, 'title');
			$class = purehearts_set( $social_icon, 'class');
			$link = purehearts_set( $social_icon, 'social_link');
			$icon = purehearts_set( $social_icon, 'social_icon');
...
add_action('purehearts_social_share_output', 'purehearts_social_share_output');
...
add_action('purehearts_social_share_output_two', 'purehearts_social_share_output_two');
...
function purehearts_get_social_icon()
{
    ...
    <?php $icons = $options->get( 'icons_social_share' );
		...
		<ul class="social-links">
			...
			<li><a target="_blank" href="<?php echo esc_url(purehearts_set( $header_social_icons, 'url' )); ?>" style="background-color:<?php echo esc_attr(purehearts_set( $header_social_icons, 'background' )); ?>; color: <?php echo esc_attr(purehearts_set( $header_social_icons, 'color' )); ?>"><i class="fa <?php echo esc_attr( purehearts_set( $header_social_icons, 'icon' ) ); ?>"></i></a></li>
            <?php endforeach; ?>
		</ul>
	<?php endif; ?>
<?php }