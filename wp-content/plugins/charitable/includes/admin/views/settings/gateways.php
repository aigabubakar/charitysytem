...
foreach ( $charitable_gateways as $charitable_gateway ) :
	...
	?>
	<?php $charitable_gateway_id = $charitable_gateway->get_gateway_id(); ?>
	<div class="charitable-settings-object charitable-gateway <?php echo esc_attr( strtolower( $charitable_gateway_name ) ); ?>">
		<span class="gateway-logo-name">
			<?php if ( isset( $charitable_gateway_doc_urls[ $charitable_gateway_id ] ) ) : ?>
				<a
					href="<?php echo esc_url( $charitable_gateway_doc_urls[ $charitable_gateway_id ] ); ?>"
					target="_blank"
					rel="noopener noreferrer"
					class="charitable-gateway-help"
					title="<?php
						/* translators: %s: gateway name (e.g. Stripe). */
						printf( esc_attr__( 'View %s documentation', 'charitable' ), esc_attr( $charitable_gateway_name ) );
					?>"
					aria-label="<?php
						/* translators: %s: gateway name (e.g. Stripe). */
						printf( esc_attr__( 'View %s documentation', 'charitable' ), esc_attr( $charitable_gateway_name ) );
	...
	<?php
	// Display Square fallback row after PayPal Commerce if the Square class
	// isn't loaded. Rendering after PayPal Commerce (rather than Stripe) keeps
	// the display order Stripe → PayPal → Square → others when the Square SDK
	// is missing and `Charitable_Gateway_Square` is therefore skipped in the
	// main loop above.
	if ( 'paypal_commerce' === $charitable_gateway->get_gateway_id() && ! class_exists( 'Charitable_Gateway_Square' ) ) :
		...
		<div class="charitable-settings-object charitable-gateway square">
			<span class="gateway-logo-name">
				<?php if ( isset( $charitable_gateway_doc_urls['square_core'] ) ) : ?>
					<a
						href="<?php echo esc_url( $charitable_gateway_doc_urls['square_core'] ); ?>"
						target="_blank"
						rel="noopener noreferrer"
						class="charitable-gateway-help"
						title="<?php esc_attr_e( 'View Square documentation', 'charitable' ); ?>"
						aria-label="<?php esc_attr_e( 'View Square documentation', 'charitable' ); ?>">
						<span class="dashicons dashicons-editor-help" aria-hidden="true"></span>
					</a>
				<?php endif; ?>
