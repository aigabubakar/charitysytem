 ...
 */
function purehearts_unset_default_widgets_args( $widget_areas ) {
...
add_filter( 'merlin_unset_default_widgets_args', 'purehearts_unset_default_widgets_args' );
 ...
 */
function purehearts_child_functions_php( $output, $slug ) {
...
add_filter( 'merlin_generate_child_functions_php', 'purehearts_child_functions_php', 10, 2 );
 ...
 */
function purehearts_local_import_files() {
	return array(
		array(
			'import_file_name'         => esc_html__('Main Demo', 'purehearts'),
			...
			'local_import_redux'       => array(
				array(
					...
					'option_name' => 'purehearts_options',
				),
			),
			'local_import_file'        => trailingslashit( get_template_directory() ) . 'demo-import/content/content.xml',
			'import_preview_image_url' => get_template_directory_uri() . '/screenshot.png',
			'import_notice'            => esc_html__( 'After you import this demo, you will have to setup the slider separately.', 'purehearts' ),
			'preview_url'              => 'http://fastwpdemo.com/newwp/purehearts/',
		),
	);
}

add_filter( 'merlin_import_files', 'purehearts_local_import_files' );

/**
 * Execute custom code after the whole import has finished.
 */
function purehearts_after_import_setup() {
	// Assign menus to their locations.
	$main_menu = get_term_by( 'name', 'Main Menu', 'nav_menu' );
	$onepage_menu = get_term_by( 'name', 'One Page Menu', 'nav_menu' );
	$rtl_menu = get_term_by( 'name', 'Main Menu', 'nav_menu' );
...
add_action( 'merlin_after_all_import', 'purehearts_after_import_setup' );